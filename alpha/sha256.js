...
export class Sha256 {
  ...
  #finish() {
    const originalBytes = this.bytesHashed;
    let index = this.bufferLength;
    this.buffer[index] = 0x80;
    index += 1;

    if (index > 56) {
      this.buffer.fill(0, index, 64);
      this.#transform(this.buffer, 0);
      index = 0;
    }
    this.buffer.fill(0, index, 56);

    const bitLengthHigh = Math.floor(originalBytes / 0x20000000);
    const bitLengthLow = (originalBytes << 3) >>> 0;
...
export async function sha256Stream(stream, onProgress) {

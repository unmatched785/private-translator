...
> **v0.1 상태:** Windows x64에서 실제 모델과 기록 흐름을 검증했지만 아직 코드 서명은 없습니다. Windows가 알 수 없는 게시자 경고를 표시할 수 있습니다.
...
1. [최신 릴리즈](https://github.com/unmatched785/private-translator/releases/latest)에서 `PrivateTranslator-v0.1.0-Windows-x64.zip`을 받습니다.
2. ZIP을 일반 폴더에 풉니다.
3. 보통은 `PrivateTranslator-Lite.exe`를 더블클릭합니다.
4. 브라우저에서 쓰는 동안 프로그램 창을 열어 둡니다. 창을 닫으면 로컬 모델도 함께 종료됩니다.

릴리즈 묶음에는 하나의 공용 모델과 두 실행 파일이 들어갑니다.
...
Quality도 인터넷 번역 API로 자동 전환하지 않습니다. 첫 릴리즈에는 용량이 큰 7B 모델을 포함하지 않습니다. Windows 10/11 x64, 시스템 메모리 8GB 이상을 권장합니다.
...
`bootstrap.ps1`은 고정된 공식 모델과 `llama.cpp` 런타임을 내려받고 크기와 SHA-256을 검증합니다. 자세한 구조와 개인정보 경계는 [architecture-v1.md](docs/architecture-v1.md), [privacy-contract.md](docs/privacy-contract.md)를 참고하세요.

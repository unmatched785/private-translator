...
실행 파일은 기본 모델 엔진의 `/health`를 먼저 확인한다. 엔진이 없으면 번들 경로에서 자동 시작하고 준비될 때까지 기다린다. Windows에서는 kill-on-close Job Object에 자식 프로세스를 넣어 앱이 강제 종료되어도 모델 엔진이 남지 않게 한다. 이미 사용자가 실행한 정상 엔진이 있으면 재사용하고 종료 시 건드리지 않는다.
